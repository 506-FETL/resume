...
assert.match(crossListSource, /handleKeyboardDrag/u)
assert.match(crossListSource, /aria-live="polite"/u)
assert.match(crossListSource, /aria-roledescription/u)
assert.match(reorderSource, /startSnapshotRef/u)
assert.match(reorderSource, /onCommitRef\.current\(next\)/u)
assert.match(reorderSource, /cancelDragging/u)
assert.match(reorderSource, /addEventListener\('pointercancel', cancelDragging/u)
...
assert.match(mobileSortSource, /wasOpenRef/u)
assert.match(mobileSortSource, /useReducedMotion/u)
...
assert.match(templateStructureSource, /useTemplateEditorStore\.getState\(\)\.manifestDraft/u)
assert.match(templateStructureSource, /activeSourceIndex/u)
...
assert.match(trackerBoardSource, /currentJob\.status === newStatus/u)
assert.doesNotMatch(trackerBoardSource, /sourceId === destinationId/u)
