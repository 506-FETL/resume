...
-- Realtime collaboration remains an explicit cross-account boundary. Edge
-- code validates the signed collaborator token, authenticated user binding,
-- active session/member leases, role and the exact resume/version scope before
-- this resolver is reached. The deny-by-default ACL pass below keeps the
-- private resolver unreachable as a direct RPC.
