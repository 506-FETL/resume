...
1. AI 原子预留、幂等请求流水、结算/释放与超时冲正。
2. 请求 ID、结构化状态日志、四类关键错误率与可用告警出口。
3. 评论写路径统一锁顺序、死锁监控和数据库契约测试。
4. 可重放数据库基线、pgTAP/空库 CI、UTC 额度语义与只读额度查询。
5. GitHub Stars 上移 Edge 缓存、幂等/限流 TTL 清理、CORS 与历史 `search_path` 收敛。
