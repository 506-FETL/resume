# 富文本协作光标原子 Widget 修复设计（已否证）

- 状态：已被真实双账号回归否证；产品已取消昵称气泡，由 `2026-07-26-collaboration-caret-label-removal-and-notification-names-design.md` 取代
- 日期：2026-07-26
- 范围：简历编辑器内 Tiptap/Yjs 远端光标 widget 的 DOM、CSS 与回归验证

## 1. 背景与现象

> 2026-07-26 真实回归记录：将 label 改为 `SPAN` 并把 caret 改为零宽
> `inline-block` 后，协作者侧仍能看到旧昵称气泡留在旧位置、新气泡出现在新位置。
> 因此“仅由 widget 几何/绘制边界不稳定导致”的假设不成立，不能继续据此增加 CSS workaround。
